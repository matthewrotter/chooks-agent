...
│  │                    AGENT RUNNER                               │   │
│  │                                                                │   │
│  │  Working directory: /workspace/group (bind-mounted from host)   │   │
│  │  Volume mounts:                                                │   │
...
Only the authentication variables (`CLAUDE_CODE_OAUTH_TOKEN` and `ANTHROPIC_API_KEY`) are extracted from `.env` and written to `data/env/env`, then mounted into the container at `/workspace/env-dir/env` and sourced by the entrypoint script. This ensures other environment variables in `.env` are not exposed to the agent. Credentials are kept out of process listings by using the env file approach.
...
1. **Ensures Docker is running** - Checks daemon status; kills orphaned NanoClaw containers from previous runs
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep nanoclaw` (macOS) or `systemctl --user status nanoclaw` (Linux) |
